...
const ErrorNameForCode = _.invert(ErrorCode);

function getCodeRuleName(code) {
    const codeName = ErrorNameForCode[code];
    return `json/${codeName ? _.kebabCase(codeName) : 'unknwown'}`;
}
...
function getDiagnostics(textDocument, jsonDocument) {
    ...
    const trailingCommaSeverity = ERROR;
    ...
    '.json': {
        ...
        postprocess: function(messages, fileName) {
            ...
            return errors.map(error => ({
                ...
                severity: error.severity === ERROR ? 2 : ERROR,
